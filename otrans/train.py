...
class Trainer(object):
    def __init__(self, params, model, optimizer, scheduler=None, is_visual=True, expdir='./',
                 ngpu=1, parallel_mode='dp', local_rank=0, mixed_precision=False, opt_level='O2'):
        ...
        if self.mixed_precision:
            ...
            self.model, self.optimizer.optimizer = amp.initialize(self.model, self.optimizer.optimizer,
...
#                 self.logger.info('[Horovod] Use %d gpus for training!' % self.ngpu)
    ...
    def train_one_epoch(self, epoch, train_loader):
        ...
        step_loss = AverageMeter() #当前步的loss
        global_flag = 1 #事件测算标志位,1为进入下一个global_step


        for step, (_, batch) in enumerate(train_loader):
        # TODO 删除
        # for step, (o, batch) in enumerate(train_loader):
            # print(f"o = {o}")
            # print(f"batch = {batch}")
            ...
            if self.mixed_precision:
                import apex.amp as amp
                with amp.scale_loss(loss, self.optimizer.optimizer, delay_unscale=False) as scaled_loss:
