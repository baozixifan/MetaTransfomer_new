...
def get_conv_mask(tensor, kernel_size, stride, padding=0, dilation=1):
    '''
    将get_enc_padding_mask后的mask进行卷积后的大小(仅时间方向，无特征方向)
    :param tensor: 输入的encoding mask最后一个维度为时间方向上的向量
    :return:对应卷积处理后的mask
    '''
    tensor[:, :, :-dilation*(kernel_size-1):stride]
