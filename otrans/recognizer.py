...
from otrans.data import BOS, EOS, normalization, compute_fbank
...
class TransformerRecognizer(object):
    ...
    def recognize(self, inputs, inputs_length,targets):
        #target为自己所加，可以删掉

        enc_states, enc_masks = self.encode(inputs, inputs_length,targets)
        ...
        with torch.no_grad():
            ...
            # print(f"type_predss = {type(predss)}")
            # for pred in predss:
            #     # print("*"*50)
            #     # print(f"pred = {pred}")
            #     pred = pred[1:]
            ...
            #         if int(i) == EOS:
            #             break
            #         predall.append(self.unit2char[int(i)])
            #         print(f"predall = {predall}")
    ...
    def encode(self, inputs, inputs_length,targets):
        #targets为自己所加，可以删掉，除了一个失败的文本监督，其他代码未用到
        enc_states, enc_mask = self.model.encoder(inputs, inputs_length,targets)
